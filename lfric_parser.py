...
def get_data(date, tmpdir):
    ...
    subprocess.check_output(['/opt/moose-client-wrapper/bin/moo', 'get',
                              'moose:/devfc/u-dh663/field.nc.file/'+date+'_lfric_slam_umf_m_*.nc',tmpdir])
    subprocess.check_output(['/opt/moose-client-wrapper/bin/moo', 'get',
                              'moose:/devfc/u-dh663/field.nc.file/'+date+'_lfric_slam_s_*.nc',tmpdir])

def check_var_meta(date,tmpdir):

#    # MLEV LIST - CURRENTLY MISSING GEOPOT, WBPT, AND TEMPERATURE. Is m_r relative humidity??
    mlev_list = ['upward_air_velocity','northward_wind','eastward_wind','potential_vorticity','m_r',
                 'combined_cloud_amount','cloud_droplet_number_concentration','m_v','m_ci','m_cl',
                 'air_potential_temperature','divergence_of_wind']
    ...
    for var in mlev_list:
        ...
        for cube in cubes:
            ...
            ml_dim = iris.coords.DimCoord(ml.points, 
                                            standard_name="model_level_number")
            time = cube.coord('time')
            time_dim = iris.coords.DimCoord(time.points, 
                                            standard_name="time", 
                                            units=time.units)
        ...
        if len(tmp_cubes.concatenate()) == 1:
            ...
            iris.save(tmp_cubes.concatenate()[0],tmpdir+'/'+date+'_'+var+'_MLEV_CUBES.nc')
        else:
    ...
    surf_list = ["air_pressure_at_mean_sea_level",
...
def main():
    ...
    update_var_in_file(conf_loc, "CSET_INITIAL_CYCLE_POINT",
                      '"'+(date_obj+datetime.timedelta(seconds=60*60)).strftime("%Y-%m-%dT%H:%M:%S")+'"')
    update_var_in_file(conf_loc, "CSET_FINAL_CYCLE_POINT",
                      '"'+(date_obj+datetime.timedelta(seconds=48*60*60)).strftime("%Y-%m-%dT%H:%M:%S")+'"')
    update_var_in_file(conf_loc, "CSET_INPUT_FILE_PATH",
                      '"'+tmpdir+'*CUBES.nc'+'"')
    update_var_in_file(conf_loc, "WEB_DIR",
                      '"'+'$HOME/public_html/CSET/LFRIC_CASESTUDIES/v0_u_dh663/inprogress/'+caseid+'_'+date+'/'+'"')

    print('Running workflow') #Around 30 mins to run with good queues.
    subprocess.check_output(['/opt/ukmo/utils/bin/cylc','vip','/home/h04/jawarner/CSET/cset-workflow'])
