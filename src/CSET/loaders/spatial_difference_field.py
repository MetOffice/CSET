...
def load(conf: Config):
    ...
    # Light rain presence.
    if conf.LIGHT_RAIN_PRESENCE_SPATIAL_DIFFERENCE:
        base_model = models[0]
        for model in models[1:]:
            yield RawRecipe(
                recipe="light_rain_presence_spatial_difference.yaml",
                variables={
                    "BASE_MODEL": base_model["name"],
                    "OTHER_MODEL": model["name"],
                    "SUBAREA_TYPE": conf.SUBAREA_TYPE if conf.SELECT_SUBAREA else None,
    ...
    # Thick fog presence.
    if conf.THICK_FOG_PRESENCE_SPATIAL_DIFFERENCE:
        base_model = models[0]
        for model in models[1:]:
            yield RawRecipe(
                recipe="thick_fog_presence_spatial_difference.yaml",
                variables={
                    "BASE_MODEL": base_model["name"],
                    "OTHER_MODEL": model["name"],
                    "SUBAREA_TYPE": conf.SUBAREA_TYPE if conf.SELECT_SUBAREA else None,
    ...
    # Snow presence.
    if conf.SNOW_PRESENCE_SPATIAL_DIFFERENCE:
        base_model = models[0]
        for model in models[1:]:
            yield RawRecipe(
                recipe="snow_presence_spatial_difference.yaml",
                variables={
                    "BASE_MODEL": base_model["name"],
                    "OTHER_MODEL": model["name"],
                    "SUBAREA_TYPE": conf.SUBAREA_TYPE if conf.SELECT_SUBAREA else None,
    ...
    if conf.SFC_WIND_GUSTS_BEAUFORT_SCALE_SPATIAL_DIFFERENCE:
        base_model = models[0]
        for model in models[1:]:
            yield RawRecipe(
                recipe="surface_wind_gusts_on_beaufort_scale_spatial_difference.yaml",
                variables={
                    "BASE_MODEL": base_model["name"],
                    "OTHER_MODEL": model["name"],
                    "SUBAREA_TYPE": conf.SUBAREA_TYPE if conf.SELECT_SUBAREA else None,
    ...
    if conf.DAILY_09_MAXIMUM_TEMPERATURE_SPATIAL_DIFFERENCE:
        ...
        # Presence of cloud base height less than 50 m.
    if conf.CLOUD_BASE_HEIGHT_LESS_THAN_50_M_SPATIAL_DIFFERENCE:
        base_model = models[0]
        for model in models[1:]:
            yield RawRecipe(
