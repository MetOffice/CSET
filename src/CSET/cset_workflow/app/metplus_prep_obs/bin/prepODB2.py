...
def read_odb_table(table: str, index: str) -> pandas.DataFrame:
    ...
    with gzip.open(path) as f:
        j = json.load(f)
    df = pandas.DataFrame(j["data"], columns=[f + f"@{table}" for f in j["fields"]])
...
def odb2ascii(
    input_pattern: Path,
    output: TextIO,
    valid_times: Iterable[metomi.isodatetime.data.TimePoint | None],
):
    """
...
) -> Iterable[metomi.isodatetime.data.TimePoint | None]:
