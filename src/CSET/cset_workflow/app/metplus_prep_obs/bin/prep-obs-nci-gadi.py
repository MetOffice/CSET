...
# Columns to output
ASCII_COLUMNS = ['Message_Type','Station_ID','Valid_Time','Lat','Lon','Elevation','GRIB_Code','Level','Height','QC_String','Observation_Value']
...
def _get_g(obsdate: datetime, system: str, type: str) -> pandas.DataFrame:
    ...
    zstdpath = '/g/data/ig2/odb2/access_{version}/{year}/{month}/{timestamp}/glm_odb2/{type}.odb.zst'
...
def varno_info() -> pandas.DataFrame:
    with open('odb2varno.json', 'rt') as f:
        j = json.load(f)
    return pandas.DataFrame(j['data'], columns=['varno_'+f for f in j['fields']]).set_index('varno_code')
...
def odb2ascii(obs: pandas.DataFrame) -> pandas.DataFrame:
    ...
    ascii['Valid_Time'] = obs['date@hdr'].astype(str) + '_' + obs['time@hdr'].astype(str).str.zfill(6)
...
def timepoint_to_datetime(tp) -> datetime:
    ...
    return datetime(tp.year, tp.month_of_year, tp.day_of_month, int(tp.hour_of_day), int(tp.minute_of_hour))
...
def main():
    ...
    parser.add_argument('--system', required=True, choices=BOM_SYSTEMS, help='Forecast system to pull observations from')
    parser.add_argument('--start', help='Start date ($CYLC_TASK_CYCLE_POINT)', default=os.environ.get('CYLC_TASK_CYCLE_POINT', datetime.now().strftime('%Y%m%dT0000Z')), type=metomi.isodatetime.parsers.TimePointParser().parse)
    parser.add_argument('--length', help='Duration of observations to load', default='PT1H', type=metomi.isodatetime.parsers.DurationParser().parse)
    ...
    with open(args.output, 'wt') as f: 
        for date in pandas.date_range(timepoint_to_datetime(start), timepoint_to_datetime(end), freq='1h'):
